...
$listadoviatico = Listar_tipo_viatico($conexion);
...
require_once 'validacion_registro_viatico.php';
require_once 'insertar_viatico.php';
          ...
          <div class="card">
            <div class="card-body">
              ...
              if (!empty($_POST['BotonRegistrar'])) {
                ...
                if (empty($Mensaje)) {
                  if (InsertarViatico($conexion) != false) {
                ...
                <div class="col-6">
                  ...
                  <select class="form-select" aria-label="Selector" id="selector" name="tipo"> <!--combobox ya cargado con las marcas traidas desde la bd -->
                    ...
                    for ($i = 0; $i < $Cantidadviatico; $i++) {
                      if (!empty($_POST['tipo']) && $_POST['tipo'] ==  $listadoviatico[$i]['ID']) { //recuerda el elemento seleccionado
                        $selected = 'selected';
                      } else {
                        $selected = ''; //limpia la variable para que solo se seleccione una opcion
                      }
                    ?>
                      <option value="<?php echo $listadoviatico[$i]['ID']; ?>" <?php echo $selected; ?>>
                        <?php echo $listadoviatico[$i]['NOMBRE'];?>
                      </option>
                    <?php } ?>
                  </select>
                </div>
