...
function DatosLogin($vUsuario, $vClave, $vConexion)
{
    ...
    // Consulta de usuario
    $SQL = "SELECT 
            U.idusuario, U.user, U.clave, U.IdEmpleado, U.Idtipo, 
            E.idempleado, E.nombre, E.apellido, E.estado, E.imagen, 
            T.idtipo, T.descripcion,
            GROUP_CONCAT(RF.nombre) AS roles_funcionales
        FROM usuario U
        JOIN empleado E ON U.IdEmpleado = E.idempleado
        JOIN tipo T ON U.Idtipo = T.idtipo
        LEFT JOIN usuario_rol_funcional URF ON U.idusuario = URF.idusuario
        LEFT JOIN rol_funcional RF ON URF.idrol_funcional = RF.idrol_funcional
        WHERE U.user = ?
    ...
    $roles = [];
    if (!empty($data['roles_funcionales'])) {
        $roles = explode(',', $data['roles_funcionales']);
    }
    ...
    if (!empty($data)) {
        ...
        elseif (
            strlen($data['clave']) === 32 && ctype_xdigit($data['clave']) &&
        ...
        if ($claveValida) {
            ...
            $Usuario['ROLES_FUNCIONALES'] = $roles;
